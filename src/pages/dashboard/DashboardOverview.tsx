...
export const DashboardOverview: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 max-w-6xl mx-auto">
      {/* Welcome Banner */}
      <div className="p-6 sm:p-8 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm border border-slate-800">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        ...
        {recentPackages.length > 0 ? (
                  ...
                  <tr key={pkg.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-5 py-4 max-w-sm">
                      <p 
                        ...
                        className="font-semibold text-slate-900 hover:text-amber-600 cursor-pointer line-clamp-1 transition-colors"
                      >
                        {pkg.metadata.titleA}
                      </p>
                    ...
                    <td className="px-5 py-4 whitespace-nowrap text-right space-x-1.5">
                      ...
                      <button
                        onClick={() => handleOpenPackage(pkg.id)}
                        className="px-3 py-1 bg-slate-900 hover:bg-slate-800 text-white rounded-md text-xs font-medium cursor-pointer transition-colors"
                      >
                        Buka 7 Blok
                      </button>
                    </td>
          ...
          <div className="text-center py-12">
            ...
            <button
              onClick={() => navigate('/generate')}
              className="mt-3 px-4 py-2 bg-slate-900 text-white text-xs font-medium rounded-full cursor-pointer"
            >
              Mulai Generate Pertama
            </button>
          </div>
        )}
      </div>
